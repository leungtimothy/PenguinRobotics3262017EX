...
task main()
{
	...
	clearLCDLine(1);
	displayLCDPos(0, 0);
	displayNextLCDString("Penguin Robotics");
	displayLCDPos(1, 1);
	displayNextLCDString("Starting Robot");
	wait1Msec(2000);
	clearLCDLine(0);
	...
	while (true)
	{
		...
		{
			startTask(autonomousLCDSelectionTask);
			...
			stopTask(autonomousLCDSelectionTask);
	  }
		...
		else
		{
			...
			startTask(driverControlLCDTask);
